...
const Chatbot = () => {
    ...
    const handleChatSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                setChatMessages(prev => [...prev, {
                    sender: 'bot',
                    text: `API Error: ${response.data.error}`,
                    isError: true
                }]);
            }
        } catch (error) {
    ...
    return (
        <div className="fixed bottom-2 right-6 w-96 h-[604px] bg-white/30 backdrop-blur-md border rounded-xl shadow-lg flex flex-col">
...
function TaskManager() {
    ...
    const analyzeStock = async () => {
        if (!ticker.trim()) {
            setAnalysis({ success: false, error: 'Ticker cannot be empty' });
            return;
        }
        if (!Number.isFinite(budget) || budget < 1000 || budget > 100000) {
            setAnalysis({ success: false, error: 'Budget must be between 1000 and 100000' });
            return;
        }

        try {
            console.log('Sending analysis request:', { ticker, budget });
